...
export default function ArticlePage() {
    ...
    useEffect(() => {
      async function fetchArticle() {
        try {
          console.log(slug)
          const res = await fetch(`/api/getArticle/${slug}`); // Calls the API route
          console.log("2")
          if (!res.ok) {
    ...
    if (!article) return (<><header className="bg-gray-100 py-4 flex items-center justify-center">
      ...
      <Loader className="mx-auto my-30 animate-spin" size="64" />
      </>
      );
    ...
    return (
      ...
      <div className="mx-8 sm:mx-40">
        <h1 className="my-4 text-center scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">
        {article.headline}
        </h1>
        <blockquote className="mt-6 border-l-2 pl-6 italic">{new Date(article.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</blockquote>
        <img src={`data:image/jpeg;base64,${article.image}`} alt="Article Image" className="mx-auto my-4" />
        <p className="whitespace-pre-line my-4 font-serif">{article.article}</p>
        </div>
